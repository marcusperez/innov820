...
function appendOptionsViewMatches(chum_profiles) {
    ...
    chum_profiles.each(function (index) {
        ...
        if (parentDiv.hasClass("blank")) {
            ...
            var checkboxTemplate = '<div style="padding: 10px;"><input type="checkbox" id="' + profileId + '" name="chum_id_' + profileId + '" value="' + profileId + '" style="width:25px;height:25px;cursor:pointer;"> Hide</div>';
...
function pofGetUserNameFromProfilePage(){
	return $(".username-bar").text().split(":")[0];
}

function pofGetMainProfilePicUrlFromProfilePage(){
	return $("#images div:first-child > a").attr("href").split(":")[2];
}
